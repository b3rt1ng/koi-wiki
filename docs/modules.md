...
Gathers basic system information from the target. On Linux: hostname, OS, kernel, architecture, uptime, CPU, RAM, disk, logged-in users, current user, shell, IP. On Windows: hostname, domain, OS version, architecture, uptime, RAM, current user, IP, enabled privileges, local users, and administrators.
...
**Platform:** Linux  
**Usage:** `run netscan <id> [--no-scan] [-s CIDR] [-t N]`
...
koi ❯ run netscan 1 -s 172.16.10.0/24 -t 2
```
...
**Platform:** Windows PowerShell  
**Usage:** `run winscalate <id>`

Checks common Windows privilege escalation vectors and displays findings by severity. Runs best on an upgraded session for complete and accurate results.

Checks performed:

- Dangerous privileges (SeImpersonatePrivilege, SeDebugPrivilege, SeTakeOwnershipPrivilege, SeLoadDriverPrivilege, SeBackupPrivilege, SeRestorePrivilege, SeCreateSymbolicLinkPrivilege)
- AlwaysInstallElevated registry key
- UAC level
- Unquoted service paths
- Writable directories in PATH
- Stored credentials (cmdkey)
- Sensitive files (unattend.xml, web.config, sysprep files...)
- Scheduled tasks running as SYSTEM with non-system executables
- AutoRun registry entries

!!! note
    Results on plain (non-upgraded) sessions may be incomplete due to response size limitations. Run `upgrade` first for accurate output.

```
koi ❯ run winscalate 2
```
...
**Platform:** Windows PowerShell  
**Usage:** `run armory <id> [-o REMOTE_DIR]`
...
```
koi ❯ run armory 2
koi ❯ run armory 2 -o "C:\Temp\tools"
```
