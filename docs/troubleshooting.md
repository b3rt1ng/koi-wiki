...
it means the current foreground shell is `cmd.exe`, not PowerShell. Koi sends commands to whatever process is reading stdin, and if that is CMD, the PS syntax is not understood.
